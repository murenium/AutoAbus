...
int main() {
    ...
    cout << "INFO: AutoAbus v2.0-PRC x64"<< endl << "license: GNU GPL v3" << endl;
    setlocale(LC_ALL, "Russian");
    ifstream settings_file;
    settings_file.open("settings.dat");
    if(!settings_file){
        cout << "Не найден файл settings.dat" << endl << "Копия программы испорчена.";
        Sleep(5000);
        return -1;
    }
    ...
    if(line == string("timeout")){
        if(line2 == string("1")) timeout_on_start = true;
            ...
            cout << "Файл settings.dat повреждён." << endl << "Копия программы испорчена.";
            Sleep(5000);
            return -1;
        }
    }
